...
# p1, dims, ad = load_model("Brownian")
# p0 = MvNormal(zeros(dims), I)
# L = LinearPath()
# ais_prob = AISProblem(p0, p1, L)
#  
# nptls = 2^8
# tk = RWMH_sweep([10^p for p in -5:0.2:1], I, 1)

# MD = MirrorDescent(stepsize = 0.005, max_Δ = 0.1, max_T = Inf)
# a_md = ais(ais_prob, MD; N = nptls, transition_kernel = tk, show_report = true)

# S = SAIS(FixedSchedule(a_md.schedule), 3)
# a_sais = ais(ais_prob, S; N = nptls, transition_kernel = tk, show_report = true)
